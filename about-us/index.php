<?php 
    $pageTitle = "About Us";
    include '../includes/header.php';
 ?>

    <div class="page-title">
            <div class="w-layout-blockcontainer container w-container">
                <div class="sub-title">
                    <img src="/assets/sub-title.svg" loading="lazy" alt="Icon"/>
                    <div>Our Story</div>
                </div>
                <h1 class="main-title">About us</h1>
            </div>
    </div>
    <div class="page-data">
        <section class="about-hero">
            <div class="w-layout-blockcontainer container w-container">
                <div class="about-wrap">
                    <div class="about-left">
                        <p class="single-text">At Blackcellence Network, we are dedicated to celebrating Black Excellence and highlighting the achievements and significant contributions of Black individuals in the Saskatchewan community.</p>
                        <p class="single-text">Each year, we partner with community-based organizations to give back through our flagship event, the Blackcellence Gala. In collaboration with a community-oriented beneficiary, the proceeds from this event are donated to ensure that our efforts directly support and uplift the local community here in Saskatchewan.</p>
                    </div>
                    
                </div>
            </div>
        </section>
        <section class="creating">
            <div class="w-layout-blockcontainer container w-container">
                <div class="creating-wrap">
                    <div class="creating-img">
                        <img src="/assets/commitment.png" loading="lazy" sizes="(max-width: 479px) 92vw, (max-width: 767px) 95vw, (max-width: 991px) 96vw, 46vw" srcset="/assets/commitment.png 500w, /assets/commitment.png 800w, /assets/commitment.png 1080w, /assets/commitment.png 1236w" alt="Commitment Image" class="creating-image"/>
                    </div>
                    <div class="creating-right">
                        <div>
                            <div class="sub-title">
                                <img src="/assets/sub-title.svg" loading="lazy" alt="Icon"/>
                                <div>Creating Change</div>
                            </div>
                            <h2 class="title">Our Goals</h2>
                        </div>
                        <div class="creating-data">
                            <div class="creating-info">
                                <h5 class="creating-title">01</h5>
                                <div class="creating-text">
                                    <h5 class="creating-title">Celebrating Blackcellence Achievements</h5>
                                    <p class="single-text mt-10">Using our platform allows for notable successes and achievements of black people in the community while giving the opportunity for the community to celebrate its champions.</p>
                                </div>
                            </div>
                            <div class="creating-info">
                                <h5 class="creating-title">02</h5>
                                <div class="creating-text">
                                    <h5 class="creating-title">Engage in and Support Community Organizational Partnerships</h5>
                                    <p class="single-text mt-10">Actively participating with community members and their Initiatives. We use this as an avenue to build new links and networks within our community with like-minded individuals and organizations.</p>
                                </div>
                            </div>
                            <div class="creating-info">
                                <h5 class="creating-title">03</h5>
                                <div class="creating-text">
                                    <h5 class="creating-title">Creating and Supporting Meaningful Connections to foster growthwithin our Community</h5>
                                    <p class="single-text mt-10">Engaging with like minded organizations to show support and assist to contribute to the initiatives they bring into our community here in Saskatchewan.</p>
                                </div>
                            </div>
                        </div>
                        
                    </div>
                </div>
            </div>
        </section>
        
        <?php include '../includes/volunteer-block.php' ?>

        <?php include '../includes/faq.php' ?>
    </div>

<?php include '../includes/footer.php'; ?>